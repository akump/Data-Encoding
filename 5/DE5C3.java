...
// DE5C3.java CS5125/6025 Cheng 2019
// RSA-PSS verification 
// Usage:  java DE5C3 publicKey signature < message 
...
public class DE5C3 {
  ...
  void readPublicKey(String filename) {
    // even when rsaKeys.txt is the argument,
    // only the public key is read (e and n)
    Scanner in = null;
    try {
  ...
  void verifyStep13() { // compare H with H2 or simply print them out
    ...
    for (; i < hLen; i++)
      if (H[i] != H2[i])
        break;
    if (i < hLen)
      System.out.println("Inconsistent: verfy step 13");
    else
  ...
  void decrypt() {
    // compute m from s : m is s modPow e n
    m = s.modPow(e, n);
    EM = m.toByteArray();
    emLen = EM.length;
  }
  ...
  public static void main(String[] args) {
    if (args.length < 2) {
      System.out.println("Usage: java DE5C3 publicKey signature < message");
      return;
    }
    DE5C3 de5 = new DE5C3();
    de5.readPublicKey(args[0]);
    de5.readSignature(args[1]);
    de5.verify();
  }
}
